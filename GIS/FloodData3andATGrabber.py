...
def query_data():
    ...
    df1 = fetch_data(f"(REQUEST_TYPE LIKE '%flood%' Or REQUEST_SUMMARY LIKE '%flood%' Or REQUEST_SUMMARY LIKE '%stand%' Or REQUEST_SUMMARY LIKE '%clog%' Or REQUEST_TYPE LIKE '%clean inlet%' Or REQUEST_SUMMARY LIKE '%clean inlet%' Or REQUEST_SUMMARY LIKE '%hotspot%') And REPORTED_DATE >= timestamp '{sqlDate} 00:00:00' And REQUEST_STATUS <> 'Closed'")
    ...
    df3 = fetch_data(f"(REQUEST_TYPE LIKE '%flood%' Or REQUEST_SUMMARY LIKE '%flood%' Or REQUEST_SUMMARY LIKE '%stand%' Or REQUEST_SUMMARY LIKE '%clog%' Or REQUEST_TYPE LIKE '%clean inlet%' Or REQUEST_SUMMARY LIKE '%clean inlet%') And REQUEST_SUMMARY NOT LIKE '%hotspot%' And REPORTED_DATE >= timestamp '{sqlDate} 00:00:00'And REQUEST_STATUS <> 'Closed'")
    ...
    resultLabel.config(text=(f"Number of Open Flooding Requests starting from {sqlDate}: {floodingRequests}\n"
...
def main():
    ...
    versionLabel = tk.Label(root, text="Version 4.2.25")
